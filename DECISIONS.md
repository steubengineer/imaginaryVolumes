...
- **Choice:** The maintainer authored two additional cyclic phase colormaps (committed 256-row RGBA
  CSVs in `tools/colormaps/`): **infinity** and **grayscale**. ADR-0014 had exactly two maps
  (twilight LUT / analytic HSV) behind a single `sampler1D`; adding more is a contract change on both
  the GPU and the host `iv::phaseColor` mirror. Menu answered by the maintainer: provide via file;
  cyclic (phase); **additive** (twilight stays default). ADR open questions accepted as-proposed
  (cycle order twilight→HSV→infinity→grayscale; grayscale on the phase axis; texel-center resampling).
- **Decision (ADR-0036):** `colormapMode` gains `2 = infinity`, `3 = grayscale` (0 twilight, 1 HSV
  unchanged; `kColormapModeCount = 4`). GPU: the single `sampler1D uColormap` becomes a
  **`sampler1DArray`**, one layer per baked LUT (layer 0 twilight, 1 infinity, 2 grayscale; HSV stays
  analytic); the shader maps `mode → layer` (`0→0, 2→1, 3→2`) and keeps linear+REPEAT on the 1-D
  coord. `colormap_lut.hpp` becomes a table (`kColormapLuts[3][256*4]` + names + counts) generated by
  `gen_colormap.py`, which now bakes twilight (matplotlib) + each `tools/colormaps/*.csv` — resampled
  at texel centers (i+0.5)/256 as a cyclic piecewise-linear map, folding the CSV's duplicated seam row
  — and gained a `--check` mode. `iv::phaseColor` selects the matching layer (host↔GPU identical data);
  the legend tracks the map for free. The viewer `C` key **cycles** all modes (was a 0↔1 toggle).
- **Rationale:** A layered LUT is the minimal scalable representation (per-layer REPEAT, no cross-map
  bleed, one binding); baking the maps as committed data keeps the host/GPU single-source invariant
  (ADR-0028) intact. Additive keeps the default and the HSV escape hatch. Texel-center resampling keeps
  every map's seam uniform and consistent with twilight.
- **Contract impact:** **ADR-0036** (Accepted 2026-07-03; amends ADR-0014, supersedes none). New
  committed data + generator; `sampler1D → sampler1DArray` (binding 3) + an `e1DArray` view / layered
  upload in the renderer; `imageBarrier` gained a defaulted `layerCount`. Public `PlotOptions`/
  `RenderParams::colormapMode` unchanged in type (now accept 0..3). Teeth: the `[vk][renderer]`
  host↔GPU cross-check now runs for **every** mode (fault-injected red→green by forcing the GPU layer
  to 0); `[transfer]` seam-continuity (all modes), grayscale + infinity anchors, and a 4-way selector;
  `gen_colormap.py --check`.
- **Deferred alternatives:** separate `sampler1D` bindings / one wide concatenated LUT / a 2-D texture
  (all rejected in the ADR); baking the CSV rows verbatim (rejected — duplicated-seam flat texel);
  caller-supplied runtime LUTs (generic **B-0005**, still open — this landed the maintainer's specific
  committed maps).

### D-0052 — Legend magnitude axis in scientific/superscript notation + content-aware placement (B-0016)
- **Date / milestone:** 2026-07-03 / post-M9 (visual-polish pass) — maintainer-directed
- **Choice:** Render the legend's auto-generated **magnitude (vertical) axis** tick labels in
  scientific/superscript form via the M9 math engine (ADR-0033), instead of ASCII `1e-3` / bare
  decimals. Format decisions (maintainer, menu): log **decade** ticks → a **pure power** `$10^{e}$`
  (the mantissa is always 1 on a decade axis, so it is elided); linear ticks stay **plain decimals**
  except for very small/large ranges (`|max| < 1e-2` or `>= 1e4`), which switch to a **shared-exponent**
  `$m\times10^{exp}$` (exp = `floor(log10|max|)`), so ticks read `4×10⁻³ / 2×10⁻³` against one power of
  ten; zero stays `0`. Public pure formatters `decadeTickLabel` / `linearTickLabel` (unit-testable).
- **Follow-on (same change):** the sci labels are **much wider** than the old decimals (a linear
  `2.5×10⁻³` ≈ 70 px vs `0.004` ≈ 37 px; a decade `10⁻⁴` ≈ 30 px is *narrower*), so the fixed
  right-edge reserve in `placeLegendRight` (`kValAllowPx = 46`) clipped the exponent (caught by a
  headless render). Fix: the facade measures the **actual widest** value label
  (`magnitudeValueReservePx(spec, glyphs)`, sharing one `magnitudeTicks(spec)` generator with
  `buildLegend`) and passes it to `placeLegendRight(…, valueReservePx)` (new defaulted arg). And the
  placement clamp is refined: when labels don't fit at the default left (`hi < default`) the swatch
  **slides left toward `hi`** to keep the widest label on screen — but **never left of the box**
  (`boxClear`); if the box fills a portrait frame so clearing it and fitting the label conflict,
  clearing the box wins and the value labels clip at the right edge (extreme-portrait degradation,
  narrow-corner: only wide sci labels; log-decade and ordinary-linear labels fit at every aspect).
- **Rationale:** Sci-notation labels are auto-generated (not caller strings), so they were out of
  ADR-0033's "caller labels are math-aware" scope — this promotes them by *generating* inline-math
  strings the existing engine typesets (no new math machinery). A content-aware reserve is the
  principled analog of `placeLegendRight` already measuring the box precisely; a fixed reserve either
  clips (wide) or wastes space (narrow). Sliding left of default (previously forbidden) is consistent
  with the pre-existing "labels win over full box clearance" priority the ADR-0034 placement test
  already encodes — the old "never left of default" pin was only right for the old narrow labels.
- **Contract impact:** **none — journaled presentation** (DEV_PROCESS §2.8; the backlog entry scoped
  B-0016 as a presentation refinement once ADR-0033's engine existed). Refines the ADR-0028 legend
  tick generation / ADR-0024 `formatTick` policy to emit math, and ADR-0034's `placeLegendRight`
  reserve/clamp — no ADR reversed, no public core-`iv` surface changed (new `iv_text` helpers only;
  `placeLegendRight` gains a defaulted arg). Teeth: `[legend]` label-format + reserve-grows tests
  (fault-injected red→green), a `[legend]` placement "slide left, never into the box" test, and the
  `[vk][plot]` legend-matches-render sample made position-robust.
- **Deferred alternatives:** a matplotlib-style factored common exponent shown once atop the axis
  (per-label shared exponent chosen instead); rescuing the value labels in extreme portrait by
  overlapping the box or shrinking the swatch (rejected — box overlap is the worse evil, D-0051).
...
- **Status:** **DEFERRED INDEFINITELY** (maintainer, 2026-07-03). The unit-cube `L` (a fraction of a
  full traversal) is acceptable as-is; the per-axis-extent convention isn't worth the plumbing now.
  Kept on the backlog (not resolved) — revive only if a physical-thickness readout is requested.
...
- **Resolved:** 2026-07-03 / **D-0052** (journaled presentation, no ADR — as anticipated). Log
  decade ticks render as pure powers `$10^{e}$`; linear ticks stay plain decimals except very
  small/large ranges, which use a shared-exponent `$m\times10^{exp}$`. Public formatters
  `decadeTickLabel`/`linearTickLabel`; the facade reserves the measured widest-label width
  (`magnitudeValueReservePx`) and `placeLegendRight` slides the swatch left to keep wide labels on
  screen (never into the box). Teeth: `[legend]` format + reserve + slide tests; `[vk][plot]`
  legend-matches-render made position-robust.

### B-0017 — Maintainer's custom colormaps
- **Origin:** maintainer, 2026-07-03 — he has developed several custom colormaps he wants to
  implement (distinct from B-0005's generic "additional cyclic colormaps"; B-0017 is his specific
  set).
- **What:** Add the maintainer's own colormaps as selectable modes alongside twilight (mode 0) / HSV
  (mode 1), for phase (cyclic) and/or magnitude. Open inputs (needed before CONTRACT): the map
  definitions (control points / formulae), count, names, cyclic vs sequential, and whether any
  replace a default or are purely additive.
- **Why open:** just requested; the next item after B-0016. Needs a CONTRACT step first.
- **Contract link:** **extends/amends ADR-0014** (colormap policy) — author the ADR amendment and get
  maintainer acceptance before coding. Implementation landmine: the committed LUT
  (`include/iv/vk/colormap_lut.hpp`, regenerated by `tools/gen_colormap.py`) is shared by the GPU
  sampler AND `iv::phaseColor`; a new map must land in BOTH in lock-step (the `[vk][renderer]`
  host↔GPU cross-check guards it). The viewer's `C` key cycles colormap modes.
- **Resolved:** 2026-07-03 / **ADR-0036** (Accepted) / **D-0053** — `colormapMode 2 = infinity`,
  `3 = grayscale` (additive; twilight/HSV unchanged). Layered LUT (`sampler1DArray`), a
  `kColormapLuts` table generated from `tools/colormaps/*.csv` (+ `gen_colormap.py --check`), host
  `phaseColor` mirrors per layer, `C` cycles all four. Teeth: per-mode host↔GPU cross-check + seam/
  anchor/selector `[transfer]` tests. Verified by eye (both maps render in the volume + legend swatch,
  seams continuous). B-0005 (generic caller-supplied maps) stays open.
