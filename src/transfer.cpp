...
// Sample the committed twilight LUT exactly as the GPU sampler1D does: 256 RGBA8 entries at
// texel centers (i+0.5)/N, linear filtering with REPEAT wrap (entry N-1 lerps back to 0).
std::array<float, 3> sampleLut(float t) noexcept {
    ...
    for (int c = 0; c < 3; ++c) {
        const float ca = static_cast<float>(iv::vk::kTwilightLut[a * 4 + c]) / 255.0f;
        const float cb = static_cast<float>(iv::vk::kTwilightLut[b * 4 + c]) / 255.0f;
